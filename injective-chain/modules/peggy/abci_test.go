...
func TestValsetSlashing_UnbondingValidator_UnbondWindow_NotExpired(t *testing.T) {
	...
	_, err := srv.Undelegate(input.Context, undelegateMsg1)
	require.NoError(t, err)
	...
	_, err = srv.Undelegate(input.Context, undelegateMsg2)
	require.NoError(t, err)
...
func TestBatchTimeout(t *testing.T) {
	...
	var (
		now                 = time.Now().UTC()
		mySender, _         = sdk.AccAddressFromBech32("inj1f2kdg34689x93cvw2y59z7y46dvz2fk8g3cggx")
...
func TestLastEventForNewValidator(t *testing.T) {
	var (
		input, ctx = testpeggy.SetupFiveValChain(t)

		validators = []sdk.ValAddress{
			testpeggy.ValAddrs[0],
			testpeggy.ValAddrs[1],
			testpeggy.ValAddrs[2],
			testpeggy.ValAddrs[3],
		}

		orchestrators = []sdk.AccAddress{
			sdk.AccAddress(validators[0]),
			sdk.AccAddress(validators[1]),
			sdk.AccAddress(validators[2]),
			sdk.AccAddress(validators[3]),
		}

		sender      = common.HexToAddress("0xf9613b532673Cc223aBa451dFA8539B87e1F666D")
		receiver, _ = sdk.AccAddressFromBech32("inj14hj2tavq8fpesdwxxcu44rty3hh90vhujaxlnz")

		amount, _ = sdk.NewIntFromString("50000000000000000000") // 50 ETH
	...
	for i, val := range validators {
		input.PeggyKeeper.SetOrchestratorValidator(ctx, val, orchestrators[i])
	}

	// 4 validators attest to a deposit event
	h := NewHandler(input.PeggyKeeper)
	for _, orch := range orchestrators {
		_, err := h(input.Context, &types.MsgDepositClaim{
			Orchestrator:   orch.String(),
			EventNonce:     1,
			TokenContract:  myErc20.Contract,
			Amount:         myErc20.Amount,
			EthereumSender: sender.Hex(),
	...
	newValInfo := testpeggy.GenerateNewValidatorInfo()

	testpeggy.AddAnotherValidator(t, input, newValInfo)

	// new validator's last claim matches attestation
	lastEvent := input.PeggyKeeper.GetLastEventByValidator(ctx, newValInfo.ValAddr)
	assert.Equal(t, uint64(1), lastEvent.EthereumEventNonce)
	assert.Equal(t, uint64(123456), lastEvent.EthereumEventHeight)
}
