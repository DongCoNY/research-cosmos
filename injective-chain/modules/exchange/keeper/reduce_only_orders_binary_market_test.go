...
var _ = Describe("Reduce-only improvements - binary markets", func() {
	...
	// 	verifyOrder(orders, orderIdx, quantity, price, isReduceOnly)
	...
	var runTest = func(file string, shouldNotFail bool) {
		filepath := fmt.Sprintf("%v/%v.json", "./scenarios/reduceonly/binary", file)
	...
	Context("Test cancel binary order", func() {
		BeforeEach(func() {
			runTest("ro1_quantity_too_big", false)
		})
	...
	// TODO RO.5 RO is cancelled when position is liquidated
	...
	Context("RO.8d new RO has better price than resting vanilla and RO orders but no RO should be cancelled", func() {
		BeforeEach(func() {
			runTest("ro8d_new_ro_better_price_no_cancel", true)
		})
		It("new RO should be placed", func() {
			verifyPosition(10, true)
			...
			verifyOrder(orders, 0, 4, 4, true)
			verifyOrder(orders, 1, 3, 5, true)
			verifyOrder(orders, 2, 3, 6, true)
			verifyOrder(orders, 3, 4, 7, false)
		})
	})
	...
	*/
	...
	Context("MARKET REDUCE-ONLY EXCEPTIONS", func() {
		...
		Context("RO25 unfilled portion of market RO is cancelled", func() {
			BeforeEach(func() {
